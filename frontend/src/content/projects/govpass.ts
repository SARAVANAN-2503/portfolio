...
export const govpass: Project = {
  ...
  explainMode: {
    ...
    talkingPoints: [
      'State machine ensures applications can only transition through valid paths, so there are no skip-ahead bugs',
    ...
    tradeoffsExplained:
      'Pure event sourcing would mean every query rebuilds state from the log, which is too slow for the officers who need instant dashboards. Dual-write trades consistency guarantees for query performance. The reconciliation job catches drift, and in 18 months of production it flagged exactly twice, both from a deploy-time race condition we subsequently fixed with a transaction lock.',
  },
};
